...
<div class="max-w-6xl mx-auto">
    ...
    <div class="card bg-base-100 shadow-xl mb-6">
        <div class="card-body">
            <form method="GET" action="{{ route('recurring-expenses.index') }}">
        ...
        <div class="card-body">
            @if ($recurringExpenses->isEmpty())
                <div class="text-center py-8">
                    <p class="text-base-content/70">No recurring expenses found.</p>
                    <a href="{{ route('recurring-expenses.create') }}" class="btn btn-primary btn-sm mt-4">Create your first recurring expense</a>
                </div>
            @else
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            @foreach ($recurringExpenses as $recurringExpense)
                                <tr>
                                    <td class="text-sm">
                                        {{ $recurringExpense->next_transaction_at->format('M d, Y') }}
                                    </td>
                                    <td class="font-medium">{{ $recurringExpense->description }}</td>
                                    <td>
                                        @if($recurringExpense->account)
                                            <a href="{{ route('accounts.show', $recurringExpense->account) }}" class="badge badge-ghost hover:badge-primary">{{ $recurringExpense->account->name }}</a>
                                        @else
                                    ...
                                    <td>
                                        <div class="flex justify-end gap-2">
                                            <a href="{{ route('recurring-expenses.show', $recurringExpense) }}" class="btn btn-ghost btn-sm tooltip" data-tip="View">
                                                <x-icons.eye class="size-4" />
                                            </a>
                                            <a href="{{ route('recurring-expenses.edit', $recurringExpense) }}" class="btn btn-ghost btn-sm tooltip" data-tip="Edit">
                                                <x-icons.pencil-square class="size-4" />
                                            </a>
                                            <x-confirm-delete :action="route('recurring-expenses.destroy', $recurringExpense)" message="Are you sure you want to delete this recurring expense?" class="btn btn-ghost btn-sm text-error tooltip" data-tip="Delete">
