...
<div class="max-w-2xl mx-auto">
    ...
    <div class="card bg-base-100 shadow-xl">
        <div class="card-body">
            <h2 class="card-title text-2xl font-bold mb-4">Edit Recurring Income</h2>

            <form action="{{ route('recurring-incomes.update', $recurringIncome) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="form-control mb-4">
                    ...
                    <input type="text" name="description" id="description"
                           value="{{ old('description', $recurringIncome->description) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div class="form-control">
                        ...
                        <input type="datetime-local" name="next_transaction_at" id="next_transaction_at"
                               value="{{ old('next_transaction_at', $recurringIncome->next_transaction_at->format('Y-m-d\TH:i')) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        ...
                        <select name="frequency" id="frequency"
                            ...
                            @foreach ($frequencies as $frequency)
                                <option value="{{ $frequency->value }}"
                                    {{ old('frequency', $recurringIncome->frequency->value) == $frequency->value ? 'selected' : '' }}>
                    ...
                    <div class="form-control">
                        ...
                        <input type="number" name="remaining_recurrences" id="remaining_recurrences"
                               value="{{ old('remaining_recurrences', $recurringIncome->remaining_recurrences) }}"
                ...
                <div class="form-control mb-4">
                    ...
                    <x-searchable-select
                        ...
                        :selected="old('account_id', $recurringIncome->account_id)"
                        placeholder="No account (skip transaction)"
                    ...
                    <select name="person_id" id="person_id"
                        ...
                        @foreach ($people as $person)
                            <option value="{{ $person->id }}"
                                {{ old('person_id', $recurringIncome->person_id) == $person->id ? 'selected' : '' }}>
                ...
                <x-tag-selector :tags="$tags" :selected="$recurringIncome->tags->pluck('id')->toArray()" />

                <div class="flex justify-end gap-2 mt-6">
                    <a href="{{ route('recurring-incomes.index') }}" class="btn btn-ghost">Cancel</a>
                    <button type="submit" class="btn btn-primary">Update Recurring Income</button>
                </div>
            </form>
        </div>
    </div>
